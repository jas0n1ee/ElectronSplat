...
type DesktopAPI = {
  ...
  runConversion: (payload:ChildJob) => Promise<{pid:number}>;
...
export type ChildJob = {
  ...
  // lodCeiling is the viewer's largest splat budget; the child refuses a job without it, because the
  // level count it plans from is what keeps the allocator able to upgrade this scene at all.
  options: { scale:number; rotation:[number,number,number]; cellSize:number; shBands:number; chunkSize:number; lodCeiling:number };
};
